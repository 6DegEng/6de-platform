...
**Already provisioned before this change?** The 2026-07-05 setup run granted a
THIRD assignment — `Contributor` on the Postgres server — which the workflow
needed only for its (now removed) firewall open/close steps. After the first
green run of this version, revoke it:

```bash
az role assignment delete --role "Contributor" --assignee "$SP_ID" \
